...
DIFFICULTY = 2 #how many moves ahead the AI will look before dtermining it's move, 0-2 only or the game will not run
...
def main():
    ...
    SONGS = ['sound/do_do_do.ogg', 'sound/new_hope.ogg', 'sound/dragon_shores.ogg', 'sound/mystic_marsh.ogg', 'sound/crystal_glacier.ogg', 'sound/balloon_chase.ogg', 'sound/chase_groove.ogg', 'sound/snow_biz.ogg']
    token_Sound = pygame.mixer.Sound('sound/dink.wav')
    ...
    REDTOKENIMG = pygame.image.load('sprites/red_token.png')
    REDTOKENIMG = pygame.transform.smoothscale(REDTOKENIMG, (SPACESIZE, SPACESIZE))
    YELLOWTOKENIMG = pygame.image.load('sprites/yellow_token.png')
    YELLOWTOKENIMG = pygame.transform.smoothscale(YELLOWTOKENIMG, (SPACESIZE, SPACESIZE))
    BOARDIMG = pygame.image.load('sprites/the_board.png')
    BOARDIMG = pygame.transform.smoothscale(BOARDIMG, (SPACESIZE, SPACESIZE))
    PLAYERWINNERIMG = pygame.image.load('sprites/player_winner.png')
    CPUWINNERIMG = pygame.image.load('sprites/cpu_winner.png')
    DRAWWINNERIMG = pygame.image.load('sprites/draw_tie.png')
    WINNERRECT = PLAYERWINNERIMG.get_rect()
    WINNERRECT.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 2))
    HELPIMG = pygame.image.load('sprites/tutorial_help.png')
    ...
    isFirstGame = True #on bootup, the tutorial will come up
    while True:
        runGame(isFirstGame)
        isFirstGame = False
    ...
    while True: 
        if turn == PLAYER:
            ...
            if isWinner(mainBoard, YELLOW):
                ...
                break
            turn = PLAYER
        score = int(pygame.time.get_ticks()/100) #time since starting in 1/10 secs
        
        if isBoardFull(mainBoard):
            winnerImg = DRAWWINNERIMG
            break

    # this is the loop when the game is over
    while True:
        if not leader_board_checked:
            if leader_board.check(score):
                leader_board.update(score)
            leader_board_checked = True
